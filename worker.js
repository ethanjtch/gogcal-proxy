// Google CalDAV Gateway for Cloudflare Workers (v6)
...
// 回调地址默认按部署域名自动推断，也可通过 env.REDIRECT 覆盖。
...
export default {
  async fetch(request, env, ctx) {
    ...
    const expected = "Basic " + btoa(cfg.GATE_USER + ":" + cfg.GATE_PASS);
    ...
    if (!token || Date.now() >= exp) {
      ...
      if (!r.ok || !data.access_token) {
        return new Response("Token 刷新失败，可能需要重新授权: " + JSON.stringify(data), { status: 401 });
      }
      token = data.access_token;
      await KV.put("access_token", token);
      await KV.put("access_expires", String(Date.now() + (data.expires_in - 60) * 1000));
    }

    // ========== 6. 代理到 Google CalDAV（用 Bearer Token） ==========
    let path = url.pathname;
    if (path === "/.well-known/caldav" || path === "/" || path === "") path = "/caldav/v2/";
    else if (!path.startsWith("/caldav/")) path = "/caldav/v2" + path;
